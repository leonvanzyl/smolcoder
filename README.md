...
- **Attach files and images.** Paste a screenshot straight from the clipboard (Ctrl+V or right-click → Paste), drop files onto the chat, or use the paperclip. Images go to the model as vision input when the model supports it (the chip warns you when it does not); text files are inlined into your message. Uploads live under `~/.smolcoder/uploads/`, never in your workspace.
